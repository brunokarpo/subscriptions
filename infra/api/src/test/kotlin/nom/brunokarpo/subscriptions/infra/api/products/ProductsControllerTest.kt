...
class ProductsControllerTest : ApiConfigurationTest() {
    @Test
    fun `should create product by name`() =
        runTest {
            ...
            coEvery { createProductUseCase.execute(any()) } returns
                CreateNewProductUseCase.Output(
                    id = expectedId,
    ...
    @Test
    fun `should return a handled error when create new product use case throw exception`() =
        runTest {
            val expectedId = "79e9eb45-2835-49c8-ad3b-c951b591bc7f"
            val expectedProductName = "Product 1"
            ...
            client
                ...
                .expectBody()
                .jsonPath("$.message")
                .isEqualTo("Product with name '$expectedProductName' already exists!")
        }
}
